...
import { X, Loader2, Trash2, Stethoscope, User, Mail, KeyRound } from 'lucide-react'
import { toggleStaffStatus, deleteStaffMember, resetStaffPassword } from './actions'

export default function EditStaffModal({ user, ownerId, onClose }: { user: any; ownerId: string; onClose: () => void }) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const result = await toggleStaffStatus(formData)
        ...
        >
            ...
            >
                ...
                <div className="relative pt-6 pb-4 px-5 text-center">
                    <div className={`w-14 h-14 rounded-full border-2 flex items-center justify-center font-bold text-lg mx-auto mb-3 ${isDoctor ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                        {user.full_name?.[0]?.toUpperCase() || user.email[0].toUpperCase()}
                    </div>
                    <p className="font-bold text-slate-800 text-base">{user.full_name || 'No Name'}</p>
                    <div className="flex items-center justify-center gap-1.5 text-xs text-slate-400 mt-0.5 mb-2">
                        <Mail size={12} />
                        <span>{user.email}</span>
                    </div>
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold border ${isDoctor ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-slate-50 text-slate-700 border-slate-200'}`}>
                        <RoleIcon size={11} /> {isDoctor ? 'Doctor' : 'Assistant'}
                    </span>
                    {isSelf && (
                        <span className="ml-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold bg-amber-50 text-amber-700 border border-amber-100">
                            You
                        </span>
                    )}
                </div>
                ...
                <form onSubmit={handleSubmit} className="px-5 pb-4 space-y-3">
                    ...
                    {isSelf && <p className="text-xs text-amber-600 text-center">You cannot modify your own account.</p>}
                    ...
                    <div className="flex gap-2 pt-1">
                        ...
                        <button type="submit" className="btn btn-primary flex-1 h-9 text-sm" disabled={loading || deleting || isSelf}>
                            {loading ? <Loader2 className="animate-spin" size={15} /> : 'Save'}
                        </button>
                    </div>
                </form>
                ...
                {!isSelf && (
                    <div className="px-5 py-3 border-t border-slate-100">
                        {!confirmDelete ? (
                            ...
                            <div className="space-y-2">
                                <p className="text-xs text-center text-red-600">Permanently delete this user? This cannot be undone.</p>
                                <div className="flex gap-2">
                                    ...
                                    <button type="button" onClick={handleDelete} className="btn flex-1 h-8 text-xs bg-red-500 text-white hover:bg-red-600 border-transparent" disabled={deleting}>
                                        {deleting ? <Loader2 className="animate-spin" size={13} /> : 'Yes, Delete'}
                                    </button>
                                </div>
                            </div>
                        )}
