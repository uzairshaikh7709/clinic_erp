    ...
    return (
        <div className="space-y-6 animate-enter">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <Link href="/superadmin/users/create" className="btn btn-primary shadow-lg shadow-blue-500/20 text-sm">
                    ...
                    {(filteredProfiles || []).map((user) => (
                        <div key={user.id} className="p-4 space-y-3">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 min-w-0">
                                    ...
                                    <div className="min-w-0">
                                        <p className="font-bold text-slate-900 text-sm truncate">{user.full_name || 'No Name'}</p>
                                        <p className="text-xs text-slate-500 truncate">{user.email}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setEditingUser(user)}
                                    className="text-slate-400 hover:text-[#0077B6] font-medium text-xs cursor-pointer flex-shrink-0"
                                >
                                    Edit
                                </button>
                            </div>
                            <div className="flex items-center gap-2 pl-12 flex-wrap">
                                ...
                                <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-semibold ${user.is_active ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${user.is_active ? 'bg-emerald-500' : 'bg-red-500'}`}></span>
                                    {user.is_active ? 'Active' : 'Disabled'}
                                </span>
                                {user.organizations?.name && (
                                    <span className="text-xs text-slate-500">{user.organizations.name}</span>
                                )}
                            </div>
                        </div>
                    ))}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {(filteredProfiles || []).map((user) => (
                                <tr key={user.id} className="hover:bg-slate-50 transition-colors group">
                                    ...
                                    <td className="px-4 lg:px-6 py-3 text-slate-500 text-xs">
                                        {user.organizations?.name || '—'}
                                    </td>
                                    <td className="px-4 lg:px-6 py-3">
